...
count <- readRDS("~/projects/Research/TCR and Gene Expression/PC distance analysis/PRJCA002413_pbmc/combine/count.rds")
barcode = colnames(count)
library(stringr)
sample = sort(unique(str_split(barcode, ":", simplify = TRUE)[,1]))
sample_mtx = str_split(sample, "_", n=2, simplify = TRUE)[,2] # exclude dataset name


for (i in seq_along(sample)) {
  bc = barcode[startsWith(barcode, sample[i])]
  if(unique(str_split(bc, ":", simplify = TRUE)[,1]) != sample[i]){
    stop("multiple samples")
  }
  mt = count[,colnames(count) %in% bc]
  colnames(mt) = str_split(bc, ":", simplify = TRUE)[,2]
  gene = rownames(mt)
  rownames(mt) = str_split(gene,":", simplify = TRUE)[,1]
  saveRDS(mt, paste0("~/projects/Research/TCR and Gene Expression/PC distance analysis/PRJCA002413_pbmc/matrix/",
                     sample_mtx[i], ".rds"))
  remove(bc)
  remove(mt)
  gc()
...
file_matrix = list.files("~/projects/Research/TCR and Gene Expression/PC distance analysis/PRJCA002413_pbmc/matrix",
                         full.names = TRUE)
file_anno = list.files("~/projects/Research/TCR and Gene Expression/PC distance analysis/PRJCA002413_pbmc/vdj/tcr",
                       full.names = TRUE)
...
for (i in seq_along(file_matrix)) {
  ...
  contig_anno = readRDS(file_anno[i])
...
saveRDS(df_immu, file = "~/projects/Research/TCR and Gene Expression/PC distance analysis/PRJCA002413_pbmc/df_immu_imputed.rds")
