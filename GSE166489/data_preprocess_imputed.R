...
folder_path <- "/home/h/hw309/projects/Research/TCR and Gene Expression/PC distance analysis/GSE166489/untar"
...
file_anno = file_anno[str_detect(file_anno, "TCR")]
sample = str_split(file_matrix, "_", 3, simplify = TRUE)[,2]

age_group = str_split(file_anno, "_", 7, simplify = TRUE)[,4]
disease_status = str_split(file_anno, "_", 7, simplify = TRUE)[,5]
recovery_status = str_split(file_anno, "_", 7, simplify = TRUE)[,6]
recovery_status = ifelse(recovery_status=="Recovered", "Yes", "No")



for (i in seq_along(file_matrix)) {
  # Load the PBMC dataset
  seurat.data <- Read10X(data.dir = file.path(folder_path, file_matrix[i]))
  if (length(seurat.data) == 2){
    seurat.data = seurat.data[["Gene Expression"]]
  }
  saveRDS(seurat.data, paste0("/home/h/hw309/projects/Research/TCR and Gene Expression/PC distance analysis/GSE166489/matrix/", sample[i], ".rds"))
  remove(seurat.data)
  gc()
  print("done")
}
...
for (i in seq_along(file_matrix)) {
  ...
  # Load the PBMC dataset
  pbmc.data <- Read10X(data.dir = file.path(folder_path, file_matrix[i]))
...
saveRDS(df_immu, file = "/home/h/hw309/projects/Research/TCR and Gene Expression/PC distance analysis/GSE166489/df_immu_imputed.rds")
