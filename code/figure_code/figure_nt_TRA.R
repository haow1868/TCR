...
# between clones
median_distance_two_clones_CD4_file = paste0(folder_path, "/", file_GSE, "/DNA/output_CD4/median_distance_two_clones_CD4_TRA.rds")
median_distance_two_clones_CD4 = median_distance_two_clones_CD4_file %>% map_dfr(readRDS)
median_distance_two_clones_CD8_file = paste0(folder_path, "/", file_GSE, "/DNA/output_CD8/median_distance_two_clones_CD8_TRA.rds")
median_distance_two_clones_CD8 = median_distance_two_clones_CD8_file %>% map_dfr(readRDS)

sample_median_CD4_file = paste0(folder_path, "/", file_GSE, "/DNA/output_CD4/sample_median_TRA.rds")
sample_median_CD8_file = paste0(folder_path, "/", file_GSE, "/DNA/output_CD8/sample_median_TRA.rds")
sample_median_CD4 = 
...
Ran_sample_median_CD4_file = paste0(folder_path, "/", file_GSE, "/DNA/output_CD4/Ran_sample_median_TRA.rds")
Ran_sample_median_CD8_file = paste0(folder_path, "/", file_GSE, "/DNA/output_CD8/Ran_sample_median_TRA.rds")
...
# supp2
pdf(paste0(folder_path, "/plots_paper/figure1_nt_TRA/trend.pdf"), width = 14, height = 12)
ggplot(data = sample_median %>% filter(distance_TRA <= 10)) +
  geom_boxplot(aes(x=factor(distance_TRA, levels = sort(unique(distance_TRA))), 
                   y=median, fill=type), outlier.shape = NA) +
  facet_wrap(~clone_type+disease, ncol=5) +
  theme_bw() +
  labs(x = "Levenshtein distance (TRA)", y = "GEX dissimilarity", 
       color="") +
...
trend_df = 
  sample_median %>%
  group_by(disease, distance_TRA, clone_type, type) %>%
  summarise(median = median(median_norm),
            .groups = "drop")

test_results <- trend_df %>%
  filter(distance_TRA <= 10) %>%
  arrange(desc(disease)) %>%
  group_by(clone_type, distance_TRA) %>%
...
test_results$distance_TRA <- factor(test_results$distance_TRA, levels = sort(unique(trend_df$distance_TRA)))
...
annotations_df <- test_results %>%
  mutate(y = 1.2,  # Adjusted y_position for text
         ...
         x = as.numeric(distance_TRA),  # Convert factor to numeric for positioning
         xmin = x - 0.2,
         xmax = x + 0.2)
...
library(ggsignif)
# new
pdf(paste0(folder_path, "/plots_paper/figure1_nt_TRA/figure1d.pdf"), width = 6.5, height =4.7)
ggplot(data = trend_df %>% filter(distance_TRA <= 10)) +
  geom_boxplot(aes(x = factor(distance_TRA), 
                   y = median,
                   fill=type), alpha = 0.8,
               outlier.shape = NA,
