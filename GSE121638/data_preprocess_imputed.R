...
# Borcherding, N., Vishwakarma, A., Voigt, A. P., Bellizzi, A., Kaplan, J., Nepple, K., ... & Zhang, W. (2021). Mapping the immune environment in clear cell renal carcinoma by single-cell genomics. Communications biology, 4(1), 122. (GEO: GSE121638)

# do not have healthy control: T:renal tumor, P:pbmc

folder_path <- "/home/h/hw309/projects/Research/TCR and Gene Expression/PC distance analysis/GSE121638/untar"
...
library(stringr)
sample = paste(str_split(file_matrix, "_", 4, simplify = TRUE)[,2],
               str_split(file_matrix, "_", 4, simplify = TRUE)[,3],
               sep="_")
...
for (i in seq_along(file_matrix)) {
  ...
  saveRDS(seurat.data, paste0("/home/h/hw309/projects/Research/TCR and Gene Expression/PC distance analysis/GSE121638/matrix/", sample[i], ".rds"))
  remove(seurat.data)
  gc()
  print("done")
}
...
saveRDS(df_immu, file = "/home/h/hw309/projects/Research/TCR and Gene Expression/PC distance analysis/GSE121638/df_immu_imputed.rds")
